...
| 2026-05-15 | **D40: v1.8 POI cache 통합 — 편지 일정 본문에 진짜 POI 이름** | QA report 의 ISSUE-002 remaining work. 이전 D39 까지는 letter 본문에 "강릉 첫 번째 자리" placeholder 표시. v1.8 에서 frontend POI cache 가 LLM 까지 흐름 정합: (a) **`UserStyle.likedPois?: Record<id, LikedPoiMeta>` 옵셔널 추가** (`shared/types/src/user-style.ts`). LikedPoiMeta = `{name, category?}`. D12 옵셔널 룰. (b) **`useUserStyle.addLike(id, meta?)`** 시그니처 확장. 기존 caller `addLike(id)` 그대로 동작. (c) **`poi-curation.tsx` ♥ 클릭** 시 POI 의 name + category 함께 박음. (d) **`RecommendRoutesInputSchema.poiNames?: Record<string, string>` 옵셔널 추가** (shared/types agent.ts). (e) **`route-select.tsx`** 가 useUserStyle.likedPois 에서 poiNames 빌드 후 streamRoutes 호출. (f) **backend `recommend-routes.ts`** 의 `stopName(poiId, order, poiNames?)` 가 poiNames 우선 lookup → mock 도 "안목 해변" 으로 채움. buildUserPrompt 가 `- {id}  (이름: {친구이름})` 형식으로 LLM 에 알려줌. (g) **`ROUTES_FRAGMENT`** 프롬프트가 user prompt 의 "(이름: ...)" 발견 시 그것을 stop.name 에 그대로 사용 명시. 결과: 편지 일정 본문에 LLM 이 "{TIME:09:30} 안목 해변에 앉아보세요" 같이 진짜 POI 이름으로 paragraph 생성 |
| 2026-05-15 | **D39: v1.7 polish 묶음 — P3 polish + SHELL C 프로필 + v2.1 편지 일정 LLM 강화** | V1.5-FOLLOWUP.md 의 P3 polish + SHELL C placeholder 채움 + 편지 일정 v2.1 강화 묶음. (a) **P3-1 theme alpha 토큰:** colors.ts 에 `amberTint` (0.10) · `amberTintSoft` (0.06) · `amberTextDeep` (#8E6A2A) · `lineStrong` (rgba(31,31,31,0.15)) 추가. home/date-weather/talk/mini-map 의 inline hex 5곳 토큰으로 통일. dark theme 도 대응값 박음. (b) **P3-2 turns persist:** session.ts partialize 에 turns 추가 (cap 50 자동, history.max 50 와 일관). 앱 재시작 후 첫 인사 매번 발생 fix. (c) **P3-3 home 최근 대화 sync:** RECENT_CHAT_DUMMY 제거 → useSession.turns 마지막 companion turn 표시 + fallback placeholder ("여행 시작하면 같이 이야기해요"). (d) **SHELL C 프로필 구현 (design-preview line 1526-1585 직역):** profile-hero (잉크 마크 40 + 이름 24px + "동행과 함께한 지 2년 4개월") + stat-row 3개 (Trips/Days/Cities, Fraunces italic 32px, useTrip.active 동적) + 나의 여행 결 (useUserStyle.tags chip list, TagChip active) + 동행 설정 4 row (placeholder text) + 보조 stats (likedCount/turnsCount). 이름 "윤서" 는 design-preview 직역 데모값 (v2 에 입력 추가). (e) **v2.1 LLM 편지 일정 강화:** `LLMLetter` type (paragraphs[] + signature) 추가, `GenerateLetterInputSchema` zod 추가 (city + dayIndex + totalDays + stops + dateLabel + userStyle). backend `llm/functions/generate-letter.ts` (D22 factory) + `LETTER_FRAGMENT` 프롬프트 (inline marker `{TIME:HH:MM}` `{PLACE:이름}` `{EM:강조어}` 형식 명시) + mock fallback (paragraphForStop friend-tone template). routes/llm.ts `POST /api/llm/letter` SSE. frontend `streamLetter` orchestrator + `LetterItinerary` 가 selectedDay 변경 시 fetch + cache + loading state. parseLetterParagraph 가 marker segment 분리해 DM Mono / italic celadon 스타일링. LLM 미응답 시 client template fallback (graceful). (f) **답장 쓰기 flow:** placeholder 였던 "↳ 답장 쓰기 · 일정 바꾸고 싶어요" → 실제 동작: turn append ("일정 바꾸고 싶어요" user 발화) + modal 닫기 + 채팅 탭 navigate (320ms slide-down 후 race 회피). 사용자가 채팅에서 후속 발화 시 streamChat 이 응답 |
| 2026-05-15 | **D38: 편지 일정 SPECIAL v1.6 — design-preview line 2074+ 직역 구현** | V1.5-FOLLOWUP.md 의 P2 결정 "v2 plan 도 정리" 후 사용자 즉시 진행 결정. v2 후순위 대신 v1.6 으로 당김. (a) **schema 확장:** `LLMRouteStop` 에 `name?: string` (POI 표시 이름) + `timeHint?: string` (HH:MM) 옵셔널 추가 (D12 옵셔널 룰). `LLMRouteStopSchema` 에 zod 일관. 기존 caller 0 영향. (b) **backend LLM 갱신:** `recommend-routes.ts` mockRoutes 가 timeHint (09:30/11:30/13:00/15:30/18:30) 채움 + buildUserPrompt 규칙에 name + timeHint 형식 추가. `update-route.ts` 도 새 stop 추가 시 마지막 timeHint+2h + name 채움. (c) **신규 컴포넌트 `modals/letter-itinerary.tsx`** (~370 lines, D38 명시적 예외): letter-header (city · Day N + 월일·요일) + day tabs (당일 N=1 hide, 2박3일 N=3) + 잉크 타임라인 (07/09/11/13/16/18/21 dots, 오늘이면 현재 시간 active marker) + letter body (paragraph-per-stop, friend-tone 1인칭 template: position+hour 별 도입부 변형 — "아침은 천천히. 11:30 안목 해변에 들렸다 갈까요?") + 서명 "— 오늘의 동행" italic celadon + 답장 placeholder (v2.1). slide-up Modal (300ms Reanimated, companion-map.tsx 패턴 재사용). (d) **`app/companion.tsx` 진입점:** activeTrip + activeRoute 둘 다 있을 때만 우하단 absolute "↳ 편지로 보기" celadon italic underline 렌더 → LetterItinerary modal open. (e) **편지 본문 생성 = client-side template** (LLM 추가 함수 없이 — `paragraphIntro/paragraphBody` 가 stop position + hour 별 friend-tone 변형 생성). v2.1 후속에서 LLM `generate-letter` 함수로 자연스러움 강화 후보 |
...
**Status: APPROVED + v1.8 POI cache (2026-05-15, D1-D40 반영, D24 skip) — v1.6 letter 머지 후 사용자 "design-preview 빠짐없이" 검토 → V1.5-FOLLOWUP P3 polish + SHELL C 프로필 + v2.1 편지 일정 LLM 강화 묶음 (D39). 19 화면 중 v1 범위 화면 모두 implementation 완료: SHELL B/B'/C ✅ · SCENARIO 01/02/02.4/02.5/03/04/06(통합)/07-CHAT/08 ✅ · ONBOARDING ✅ · TRIP START / ON-TRIP COMPANION / 지도 모달 ✅ · ★ SPECIAL 편지 일정 (LLM friend-tone) ✅. v2 lock: SHELL A 로그인 · SCENARIO 07-VOICE · SCENARIO 07-DIARY · 예약 (D32 음성 종속). 잔여 ~1주 (Phase 6 검증 + Phase 7 TestFlight).**
