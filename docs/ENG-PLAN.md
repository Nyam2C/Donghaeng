...
**Status: APPROVED + v1.6 letter (2026-05-15, D1-D38 반영, D24 skip) — Phase 4a-5b + v1.5 flow 통합 (D37) 머지 후 사용자 즉시 결정: 편지 일정 SPECIAL v2 → v1.6 당김. D38 구현 (modals/letter-itinerary.tsx + LLMRouteStop 옵셔널 확장 + companion 진입점). v1 핵심 흐름 + SPECIAL 편지 일정 (다른 어떤 여행 앱에도 없는 형태) 모두 v1.6 안. SCENARIO 06 돌발리플랜 별도화면만 v2 lock. 잔여 ~1.5주 (Phase 6 검증 + Phase 7 TestFlight).**
